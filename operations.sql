...
--3.3.2 перед закрытием склада нужно выключить внешние с ним ключи
alter table item_list disable constraint item_list_storage_id_fk;
alter table item_list disable constraint item_list_storage_id_nn;

--произвести закрытие
delete from storages
where id=2; --отработает триггер delete_storage (закрытие склада)
--3.3.3 и переместит все товары из него в другой склад

--затем обратно включить ограничения
alter table item_list enable constraint item_list_storage_id_fk;
alter table item_list enable constraint item_list_storage_id_nn;